                ...
                {products.map((product, id) => (
                    <div onClick={() => navigate(`/user/singleProduct/${product._id}`)} onMouseOver={() => setHoveredProduct(id)}
                        ...
                        <div className='p-4 relative  w-full '>
                            {!(hoveredProduct === id) && <span className={`text-xs border-1 border-gray-400 bg-black text-white p-1 absolute top-5 left-5 shadow-md transform transition-transform translate-x-4`}>
                                {product.brand}
                            </span>}
                            
                            {wishlist.length === 0 ? <FaRegHeart className='absolute  top-5 right-5 h-7 text-red-500 w-7 hover:cursor-pointer' onClick={(e) => {
                                e.stopPropagation();
                            ...
                            }} className={`absolute  top-5 right-5 h-7 text-red-500 w-7 hover:cursor-pointer ${
                                animateHeart ? 'animate-pulse': ''}`}/> : <FaRegHeart className='absolute  top-5 right-5 h-7 text-red-500 w-7 hover:cursor-pointer' onClick={(e) => {
                                e.stopPropagation();
                                handleHeartIconClick(product._id, product.price);
                            }} />}</>}

                            <span className='absolute bottom-[26px] right-7 flex  bg-slate-100 items-center  p-1 font-mono text-[12px]'>4.4<IoStar className='h-3 m-1 w-3' /></span>
